    ...
    <div class="weui_tab">
        ...
        <div class="content-inner">
            <div class="video-wrap clearfix">
                <div id="videoarea" style="position:relative;">
                    <img src="http://pic32.photophoto.cn/20140707/0006019627979225_b.jpg" alt="xxx小课堂" width="100%">
                </div>
            </div>
            <ul class="course-tab">
                <li class="curr">详情</li>
        ...
        <div class="spec-menu-content spec-menu-show" style="display: none;">
            <div class="spec-menu-top bdr-b">
                <div class="spec-first-pic">
                    <img id="spec_image" src="http://pic32.photophoto.cn/20140707/0006019627979225_b.jpg" onerror="imgErr(this)">
                </div>
                        ...
                        if (data.result === 'success') {
                            ...
                            wx.chooseWXPay({
                                ...
                                success: function (res) {
                                    // 支付成功后的回调函数
                                    alert('支付成功！' + res);
                                    setTime("location.href = '${contextPath!}/weixin/h5/queryCoursePlayer.do?id=${info.id!}'", 1000);
                                }
                            });

                            wx.chooseWXPay({
                                appId: 'wx7ee19649ab15e945',
                                timestamp: '1525887502', // 支付签名时间戳，注意微信jssdk中的所有使用timestamp字段均为小写。但最新版的支付后台生成签名使用的timeStamp字段名需大写其中的S字符
                                nonceStr: '2Blkqujtl2sUf08C', // 支付签名随机串，不长于 32 位
                                package: 'prepay_id=wx10013822618446b4eebc49cd3290736809', // 统一支付接口返回的prepay_id参数值，提交格式如：prepay_id=\*\*\*）
                                signType: 'MD5', // 签名方式，默认为'SHA1'，使用新版支付需传入'MD5'
                                paySign: 'FC91253F87CF5972C1117E18F1193ADD', // 支付签名
                                success: function (res) {
                                    // 支付成功后的回调函数
                                    alert('支付成功！' + res);
                                    setTime("location.href = '${contextPath!}/weixin/h5/queryCoursePlayer.do?id=${info.id!}'", 1000);
        ...
        wx.ready(function(){
            var shareData = {
                title: "xxx小课堂",
                desc: "这是xxx小课堂",
                link: "http://ketang.hbgstzgl.com/weixin/h5/main.do",
