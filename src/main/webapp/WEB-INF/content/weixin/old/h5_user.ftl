...
<div class="head">
    ...
    <p class="money">奖励金额：<span>${(weixinUser.promoterMoney)!}元</span></p>
    <p class="money_rule">
...
<div class="weui_panel weui_panel_access">
    <div class="weui_panel_hd">我的分销记录</div>
...
<script type="text/javascript">
    var ajaxUrl   = BASE_PATH + "/weixin/h5/queryOrderListJson.do";
    ...
    $(function () {
        ...
        function insertDiv(resultObj) {
            ...
            $.each(result, function (j, info) {
                chtml += '<a href="' + lessonUrl + '&id=' + info.courseId + '" class="weui_media_box weui_media_appmsg">';
                chtml += '    <div class="weui_media_hd">';
                chtml += '        <img class="weui_media_appmsg_thumb" src="' + BASE_IMG_PATH + info.logoUrl + '" alt="">';
                chtml += '    </div>';
                chtml += '    <div class="weui_media_bd">';
                chtml += '        <h4 class="weui_media_title">';
                if(info.status > '2'){
                    chtml += '        ' + info.courseName + '<span class="green">（' + info.statusName + '）</span>';
                }else{
                    chtml += '        ' + info.courseName + '<span class="red">（' + info.statusName + '）</span>';
                }
                chtml += '        </h4>';
                chtml += '        <p class="weui_media_desc">' + info.body + '</p>';
                chtml += '    </div>';
                chtml += '</a>';
            });
