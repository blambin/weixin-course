...
function OrganSelect(){
	...
	function organChange(obj, dv,status) {
		if (undefined == dv) {
			dv = '';
		}
		var $obj = jQuery(obj);
		...
		if ($obj.val() == '') {
			...
			if (items.length == 0) {
				// 创建下一元素
				createNextItem(BASE_PATH+'/base/user/ajax.do', $obj.val(), dv, $obj
						.parent().attr("id"),status);
			} else {
				for (var i = 0; i < items.length; i++) {
					...
					if (item.attr('data-parent') == '1') {
						...
						// 重新取值
						item.remove();
						createNextItem(BASE_PATH+'/base/user/ajax.do', $obj.val(),'', $obj.parent().attr("id"),status);
					}
				}
			}
		}
	...
	// 创建下一元素
	function createNextItem(url, _organId, dv, showAreaId,status) {
		$.ajax({
			url : url + "?organId=" + _organId + "&defVal=" + dv + "&status=" +status,
			type : 'POST',
			async : true,
		...
		if (val!='' && val.length > 0) {
			...
			for (var i = 0; i < arr.length; i++) {
				if (i < sel.length) {
					...
					// 门店及已下的需要判断 added by yipan 2016-04-27 10:58:57
					if (null != (sel[i - 1])) {
