 ...
 */
function showMenu() {
		var companyid = jQuery("#companyId");
		var cityOffset = jQuery("#companyName").offset();
		var zTree = jQuery.fn.zTree.getZTreeObj("treeDemo");
		if(companyid.val().length>0){
			var no = zTree.getNodeByParam("id", companyid.val());
			if (no != null) {    
		...
		jQuery("#menuContent").css({left:cityOffset.left + "px", top:cityOffset.top + 25 + "px"}).slideDown("fast");
	...
	function onBodyDown(event) {
		if (!(event.target.id == "companyName" || event.target.id == "menuContent" || jQuery(event.target).parents("#menuContent").length>0)) {
			hideMenu();
		}
	...
	function onCheck(e, treeId, treeNode) {
		...
		if(treeNode.getCheckStatus().checked){
			jQuery("#companyName").val(treeNode.name)
			jQuery("#companyId").val(treeNode.id);
